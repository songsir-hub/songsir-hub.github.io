...
第二台电脑验证时，`[1]` 项显示系统代理被设为一个残留的公网代理地址（非 FastGithub 设置）。透明模式本就不需要系统代理，这个残留代理会把 GitHub 流量引向别处，使加速形同虚设。增加 `clear_proxy.bat`（关闭 WinINet 代理 + `netsh winhttp reset`）一键清除后，验证恢复正常。
...
- **可复用技能**（本地沉淀，未对外公开）：记录全部坑点与已修正的脚本（含被替换的 `cmd /k` 守卫），避免旧 bug 复现。
